...
const LockerController = require('../controllers/api/v1/lockersController');
...
describe('Lockers controller', function(){
  ...
  describe('getLockers', function(){
    it('should return lockers with status code 200', function(done) {
      const req = httpMocks.createRequest({ session: { user: user }, params: { tower_id: tower._id } })
      const res = httpMocks.createResponse();
      LockerController.getLockers(req, res, () => {}).then(() => {
        const responseData = res._getJSONData();
        expect(responseData).to.eql('Test')
